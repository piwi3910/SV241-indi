...
bool SV241::initProperties()
{
    ...
    PWM14NP[0].fill("PWM14_VALUE", "Value (0-253)", "%.0f", 0, 253, 1, 0);
    PWM14NP.fill(getDeviceName(), "PWM14", "PWM14 (Dew Heater 1)", MAIN_CONTROL_TAB, IP_RW, 60, IPS_IDLE);

    PWM15NP[0].fill("PWM15_VALUE", "Value (0-253)", "%.0f", 0, 253, 1, 0);
    PWM15NP.fill(getDeviceName(), "PWM15", "PWM15 (Dew Heater 2)", MAIN_CONTROL_TAB, IP_RW, 60, IPS_IDLE);
...
bool SV241::syncAllStates()
{
    ...
    // Update UI state
    DC1SP[0].setState(dcStates[CH_DC1] ? ISS_ON : ISS_OFF);
    DC1SP[1].setState(dcStates[CH_DC1] ? ISS_OFF : ISS_ON);
    DC1SP.setState(IPS_OK);
    DC1SP.apply();

    DC2SP[0].setState(dcStates[CH_DC2] ? ISS_ON : ISS_OFF);
    DC2SP[1].setState(dcStates[CH_DC2] ? ISS_OFF : ISS_ON);
    DC2SP.setState(IPS_OK);
    DC2SP.apply();

    DC3SP[0].setState(dcStates[CH_DC3] ? ISS_ON : ISS_OFF);
    DC3SP[1].setState(dcStates[CH_DC3] ? ISS_OFF : ISS_ON);
    DC3SP.setState(IPS_OK);
    DC3SP.apply();

    DC4SP[0].setState(dcStates[CH_DC4] ? ISS_ON : ISS_OFF);
    DC4SP[1].setState(dcStates[CH_DC4] ? ISS_OFF : ISS_ON);
    DC4SP.setState(IPS_OK);
    DC4SP.apply();

    DC5SP[0].setState(dcStates[CH_DC5] ? ISS_ON : ISS_OFF);
    DC5SP[1].setState(dcStates[CH_DC5] ? ISS_OFF : ISS_ON);
    DC5SP.setState(IPS_OK);
    DC5SP.apply();

    USB12SP[0].setState(dcStates[CH_USB12] ? ISS_ON : ISS_OFF);
    USB12SP[1].setState(dcStates[CH_USB12] ? ISS_OFF : ISS_ON);
    USB12SP.setState(IPS_OK);
    USB12SP.apply();

    USB345SP[0].setState(dcStates[CH_USB345] ? ISS_ON : ISS_OFF);
    USB345SP[1].setState(dcStates[CH_USB345] ? ISS_OFF : ISS_ON);
    ...
    double pwm13Voltage = (pwmValues[0] * 15.3) / 253.0;
...
bool SV241::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (USB12SP.isNameMatch(name))
        {
            ...
            if (setDCOutput(CH_USB12, enabled))
            {
                USB12SP.setState(IPS_OK);
            }
            else
        ...
        if (USB345SP.isNameMatch(name))
        {
            ...
            if (setDCOutput(CH_USB345, enabled))
            {
                USB345SP.setState(IPS_OK);
            }
            else
